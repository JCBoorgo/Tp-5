...
public class ListeMatricesChiffrement_SimpleTest {
    ...
    private static final int DIM_MATRICE = 4;
	...
	System.out
		.println("Les méthodes sur l'objet ListeMatricesChiffrement ...");
	...
	System.out.println("getNombreMatrices : " + mc.getNombreMatricesCandidates());
	System.out.println("getMatriceCourante : \n"
		+ MatriceUtilitaires.toStringMat(mc.getCopieMatriceCourante()));
	System.out.println("getMatriceCouranteInverseHill : \n"
		+ MatriceUtilitaires.toStringMat(mc
			.getMatriceCouranteInverseHill()));

	System.out
		.println("Boucle pour voir le PGCD de chaque matrices candidates avec le coefficient dans Z, devrait toujours être = 1 ...");
	for (int i = 0; i < mc.getNombreMatricesCandidates(); i++) {
	    ...
	    System.out
		    .print("[mat "
			    + i
			    + " : "
			    + MathUtilitaires.PGCD(MatriceUtilitaires
				    .getDeterminant(mc.getCopieMatriceCourante()),
				    COEFFICIENT_DANS_Z) + "], ");
	}

	System.out
		.println("\n\nBoucle pour voir 4 matrices ou moins de l'ensemble au hasard ...");
	for (int i = 0; i < ((4 <= mc.getNombreMatricesCandidates()) ? 4 : mc
		.getNombreMatricesCandidates()); i++) {
