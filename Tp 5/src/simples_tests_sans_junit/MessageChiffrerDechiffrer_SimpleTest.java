...
public class MessageChiffrerDechiffrer_SimpleTest {
    ...
    private static SortedSet<String> dico = FichierUtilitaires
	...
	if ((message = InputUtilitaires.saisirString(
		"Entrez un message à chiffrer (sans accent)", "Saisir message")) != null) {

	    listeDeMatricesCandidates = new ListeMatricesChiffrement(MIN, MAX,
		    DIM_MAT, vecCaracteres.getTaille());

	    mesChiffrerDechiffrer = new MessageChiffrerDechiffrer(
		    vecCaracteres, listeDeMatricesCandidates, dico);

	    System.out.println("Vecteur de caractères utilisé : "
		    + vecCaracteres);
	    System.out.println("Nombre de matrices "
	    ...
	    System.out.println("Message à chiffrer : [" + message
		    + "] - d'une longueur de " + message.length()
		    + " caractères.");

	    System.out.println();
	    message = mesChiffrerDechiffrer.encoder(message);
	    System.out
		    .println("Matrice choisie au hasard, pour le chiffrement, parmi les matrices candidates :\n"
			    + MatriceUtilitaires
				    .toStringMat(mesChiffrerDechiffrer
					    .getMatriceCourante()));
	    System.out.println("Message encoder: [" + message
		    + "] - d'une longueur de " + message.length()
		    + " caractères.");
		...
		System.out
			.println("Matrice trouvée, pour le déchiffrement, parmi les matrices candidates :\n"
				+ MatriceUtilitaires
					.toStringMat(mesChiffrerDechiffrer
						.getMatriceCourante()));

		System.out.println("Message decoder: [" + message
			+ "] - d'une longueur de " + message.length()
			+ " caractères.");
	    } else
		System.out
			.println("Aucune matrice trouvée pour déchiffrer le message...");
     ...
     * [16, 17, 18, 20]
     * 
     * Message encoder: [BHSVB-UPQIKQGGYAQ-XHVDAMRSTGDADS] - d'une longueur de 32 caractères.
     * 
     ...
     * [16, 17, 18, 20]
     * 
     * Message decoder: [UN PETIT MESSAGE POUR LE CODAGEX] - d'une longueur de 32 caractères.
     * </pre>
     */
}
