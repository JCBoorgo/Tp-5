...
public class ChiffrementDeHill extends JPanel implements ActionListener {
	...
	private SortedSet<String> dico = FichierUtilitaires.lireDictionnaire(new File(NOM_FICHIER_DICO));
	private MessageChiffrerDechiffrer mesChiffrerDechiffrer = null;
	private String[] proverbes = FichierUtilitaires.lireDictionnaire(new File(NOM_FICHIER_PROVERBES))
	...
	public ChiffrementDeHill() {
		...
		listeDeMatricesCandidates = new ListeMatricesChiffrement(BORNE_MIN, BORNE_MAX, DIM_MAT,
		...
		textInfo = "-- Vecteur de caractéres : " + vecCaracteres + ".\n";
		textInfo += "-- Les combinaisons de " + BORNE_MIN + " à " + BORNE_MAX + " donnent "
				+ listeDeMatricesCandidates.getNombreMatricesCandidates() + " matrices candidates de "
				+ listeDeMatricesCandidates.getDimension() + " X " + listeDeMatricesCandidates.getDimension() + ".\n";
	...
	private void initMenuBar() {
		...
		String titresMenuFichier[] = { "Ouvrir message gris...", "Enregistrer sous message gris...", "Quitter" };
		...
		String titresMenuAction[] = { "Chiffrement sans saisie", "Chiffrement avec saisie...", "Dechiffrement",
				"Vider zone de texte" };
	 ...
	 * 
	 * @param event
	 *            , l'évenement qui a sollicité la méthode. Permet de connaître
	 *            le composant graphique stimulé.
	 */
	@Override
	...
	private void gestionOuvrirMessageGris() {
		...
		if (fichier != null) {
			...
			this.messageBlanc = "";
			this.sortieHTML += messageTitre("--- Opération de lecture d'un message gris...");
		} else
			this.sortieHTML += messageWarning("Désolé, l'opération de lecture d'un message gris a été annulée...")
					+ "<br />";
	...
	private void gestionEnregistrerSousMessageGris() {
		if ((this.messageGris != null) && !this.messageGris.isEmpty()) {
			File fichier = FichierUtilitaires.obtenirNomFichier("Enregistrer",
					new File(this.messageBlanc + EXT_FICHIER));

			if (fichier != null) {
				FichierUtilitaires.enregistrerMessage(messageGris, fichier);
				this.sortieHTML += messageTitre("--- Opération d'enregistrement d'un message gris...");
			} else
				this.sortieHTML += messageWarning(
						"Désolé, l'opération d'enregistrement d'un message gris a été annulée...") + "<br />";
		} else
			this.sortieHTML += messageWarning("Désolé, pas de message gris `enregistrer pour le moment...") + "<br />";
	...
	public void gestionQuitter() {
		...
		if ((this.messageGris != null) && !this.messageGris.isEmpty()) {
			choix = JOptionPane.showConfirmDialog(this, messageBoite, titreBoite, JOptionPane.YES_NO_OPTION);
	...
	private void gestionChiffrementSansSaisie() {
		if ((this.proverbes != null) && (this.proverbes.length > 0)) {
			this.messageBlanc = proverbes[MathUtilitaires.alea(0, (proverbes.length - 1))].trim().toLowerCase();
			this.messageGris = this.mesChiffrerDechiffrer.encoder(messageBlanc);
			this.sortieHTML += messageTitre("--- Opération de chiffrement sans saisie...");
		} else

			this.sortieHTML += messageWarning("Désolé, aucun message disponible dans le tableau interne de messages...")
					+ "<br />";
	...
	private void gestionChiffrementAvecSaisie() {
		...
		this.messageBlanc = InputUtilitaires.saisirString(messageBoite, titreBoite);

		if ((this.messageBlanc != null) && !this.messageBlanc.isEmpty()) {
			this.messageBlanc = this.messageBlanc.trim().toLowerCase();
			this.messageGris = this.mesChiffrerDechiffrer.encoder(messageBlanc);
			this.sortieHTML += messageTitre("--- Opération de chiffrement...");
		} else {
			this.messageBlanc = "";
			this.sortieHTML += messageWarning("Désolé, aucun message saisi...") + "<br />";
		}
	...
	private void gestionDechiffrement() {
		...
		if ((this.messageGris != null) && !this.messageGris.isEmpty()) {
			...
			this.sortieHTML += messageTitre("--- Opération de déchiffrement...");
		} else
			this.sortieHTML += messageWarning("Désolé, aucun message à déchiffrer...") + "<br />";
	...
	private void ajouterASortieHTML() {
		this.sortieHTML += messageInfo("<b>Message blanc : </b>[" + this.messageBlanc + "]") + "<br />";
		this.sortieHTML += messageInfo("<b>Message gris : </b>[" + this.messageGris + "]") + "<br />";
	}
